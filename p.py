...
async def gpt_command(client: Client, message: Message):
    if message.reply_to_message and len(message.command) == 1:
        ...
        await message.reply(f"<b>Usage:</b> <code>{prefix}gpt [prompt]</code> or reply to a message with <code>{prefix}gpt</code>")
        return
    await fetch_copilot_response(query, message, message.from_user.is_self)

modules_help["gpt"] = {
    "gpt [query]*": "Ask anything to GPT (via Copilot API)",
}
